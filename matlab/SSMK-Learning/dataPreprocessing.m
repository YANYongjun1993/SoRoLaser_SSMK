...
IMInfo.trans.map = @(x) transpose(tanSpace0)*tanSpace0_not_orth*x;
...
for i = 1:nPoints
    ...
    dataDecay{i} = struct('t', zDataCrt{i,1}', 'y', zDataCrt{i,2}', 'x', oDataDelay{i,2}(outdofsDelay,:)');
end
...
SSMDim = 2;
overEmbed = 32;
[crtlDataDelay, ~] = coordinatesEmbeddingControl(zu, SSMDim, 'OverEmbedding', overEmbed);
embedDim = size(crtlDataDelay{1,2},1);
outdofsDelay = [embedDim-6 embedDim-5 embedDim-4 embedDim-3 embedDim-2 embedDim-1 embedDim];
%% Mapping
Yu = crtlDataDelay; 
load SSM_model.mat
...
lowDimDelay = projectTrajectories(ssm_model.IMInfo, Yu); % Project down
newLowDim = ssm_model.IMInfo.trans.map(lowDimDelay{1,2});
obsRec2 = ssm_model.IMInfoCtrl.parametrization.map(newLowDim);
% sizeAfterEmbed = size(CtrlData1{1,2},2);
% sizeBeforeEmbed = size(keptIdxs,2);
% pack the trajectory and control input data at the low dimentional as struct
CtrlData1Packed = struct('t', Yu{1,1}', 'u', uCell{1,2}(:,(size(zu{1,1},2)-size(newLowDim,2)+1):end)', 'y', newLowDim','x',newLowDim');
ObsEmdbCtrl{1} = zu{1,2}';
...
uCell{1,1} = zu{1,1}; uCell{1,2} = uMotorCmd(:, keptIdxs); 
%% Delay-embedding with the data after smothing process
SSMDim = 2;
overEmbed = 32;
[crtlDataDelay, ~] = coordinatesEmbeddingControl(zu, SSMDim, 'OverEmbedding', overEmbed);
embedDim = size(crtlDataDelay{1,2},1);
...
Yu = crtlDataDelay; 
load SSM_model.mat
CtrlData2 = projectTrajectories(ssm_model.IMInfoCtrl, zu); % Project down
obsRec1 = ssm_model.IMInfoCtrl.parametrization.map(CtrlData2{1,2});
lowDimDelay = projectTrajectories(ssm_model.IMInfo, Yu); % Project down
newLowDim = ssm_model.IMInfo.trans.map(lowDimDelay{1,2});
obsRec2 = ssm_model.IMInfoCtrl.parametrization.map(newLowDim);
% pack the trajectory and control input data at the low dimentional as struct
CtrlData2Packed = struct('t', Yu{1,1}', 'u', uCell{1,2}(:,(size(zu{1,1},2)-size(newLowDim,2)+1):end)', 'y', newLowDim','x',newLowDim');
ObsEmdbCtrl{2} = zu{1,2}';
...
Yu = crtlDataDelay; 
load SSM_model.mat
CtrlData3 = projectTrajectories(ssm_model.IMInfoCtrl, zu); % Project down
obsRec1 = ssm_model.IMInfoCtrl.parametrization.map(CtrlData3{1,2});
lowDimDelay = projectTrajectories(ssm_model.IMInfo, Yu); % Project down
newLowDim = ssm_model.IMInfo.trans.map(lowDimDelay{1,2});
obsRec2 = ssm_model.IMInfoCtrl.parametrization.map(newLowDim);
% pack the trajectory and control input data at the low dimentional as struct
CtrlData3Packed = struct('t', Yu{1,1}', 'u', uCell{1,2}(:,(size(zu{1,1},2)-size(newLowDim,2)+1):end)', 'y', newLowDim','x',newLowDim');
ObsEmdbCtrl{3} = zu{1,2}';
...
% Create the dynamic file name with the folder path
fileName = fullfile('processedData', sprintf('figure8SSMK_%s.mat', current_date));

% Save the trajectory data
save(fileName, 'lowDimCtrl');

% Create the dynamic file name with the folder path
fileName = fullfile('processedData', sprintf('figure8SSMKOBsEmbd_%s.mat', current_date));

% Save the trajectory data
save(fileName, 'ObsEmdbCtrl');
%%
...
% Create the dynamic file name with the folder path
fileName = fullfile('processedData', sprintf('softSSMK_figure8_train-%d_val-%d_%s.mat', a, b, current_date));
