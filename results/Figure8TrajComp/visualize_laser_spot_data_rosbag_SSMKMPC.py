...
def load_reference_trajectory(mat_file="figure8_extracted_data.mat"):
    try:
        ...
        figure8_x = data['figure8_x'].flatten() - 3.5
...
def visualize_multiple_trajectories(results_dict):
    ...
    timestamps, actual_coords = slice_motion_data(timestamps, actual_coords, 236, 700)
    ...
    plot_filename = f"SSMK_actual_coordinate_trajectory_{current_date}.png"
...
def visualize_time_series(results_dict):
    ...
    if len(timestamps) > 0:
        ...
        if ref_x is not None:
            ax2.plot(ref_t_all, ref_x_all, color='gray', linestyle='--', linewidth=2, label='Reference Trajectory', alpha=0.8)
    ...
    # Save the plot
    current_date = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_filename = f"SSMK_time_series_coordinates_{current_date}.png"
    plt.savefig(plot_filename, dpi=600, bbox_inches='tight')
...
def main():
    ...
    file_configs = [
        {
            'mcap_file': "rosbag_20250817_160507_0.mcap",
            'mat_file': "SSMK.mat",  # Changed from json_file to mat_file
