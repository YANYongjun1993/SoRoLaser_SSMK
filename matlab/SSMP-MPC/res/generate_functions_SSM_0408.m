clc
clear all
load SSM_model_0405.mat;
size_x = 3;                   % number of states
size_u = py_data.params.input_dim;                   % number of control input
size_f = 3;                   % number of reference
dis = 110;                     % location of the work space, unit: mm
[size_C, size_Cbar]=generate_functions(size_x,size_u,size_f,dis,py_data);


function [size_C, size_Cbar]=generate_functions(size_x,size_u,size_f, dis_z_axis,py_data)
%%-------------------------------------------------------------------------        
...
% Revision history: The reference is changed as the traajectory at the SSM
syms x u ref xx
for n = 1:1:size_x
...
for l = 1:1:size_f
    syms (sprintf('ref%01d',l));
    ref(l) = sym(sprintf('ref%01d',l));
end
for l = 1:1:6
    syms (sprintf('xx%01d',l));
    xx(l) = sym(sprintf('xx%01d',l));
end
...
obs2lowdim = py_data.model.IMInfoInv.parametrization.map([xx1;xx2;xx3;xx4;xx5;xx6]);
obs2lowdim_func = matlabFunction(obs2lowdim);
matlabFunction(obs2lowdim_func, 'file', 'obs2lowdim_func','vars',xx);

% Simplify coefficients on symbolic fraction
digitsOld = digits(3);
% x_lowDim_Sim = vpa(x_lowDim,3);

Dx_lowDim = py_data.model.RDInfo.reducedDynamics.map([x1;x2;x3])*0.5 + py_data.model.Br*[u1;u2;u3;u4];
% Dx_lowDim = py_data.model.RDInfo.reducedDynamics.map([x1;x2;x3]) + py_data.model.Br*[u1;u2;u3;u4];
Dx_lowDim_Sim = vpa(Dx_lowDim,5);


% Equations of system dynamics
f_Org = diag([0.8,1,0.4])*x' + py_data.model.Ts*Dx_lowDim_Sim;
% f_Org = x' + py_data.model.Ts*Dx_lowDim_Sim;
f = vpa(f_Org,3); 
% f = x'*0.8 + py_data.model.Ts*Dx_lowDim*0.8;

% x_highDim = py_data.model.IMInfo.parametrization.map(f);
% 
% x_highDim_Sim = vpa(x_highDim,5);


% tip_position_x = x_highDim_Sim(4,1);
% tip_position_y = x_highDim_Sim(5,1); 
% tip_position_z = x_highDim_Sim(6,1);
...
% Calculate the laser dot position and velocity in x axis and y axis, based on the bending angles
...
% Terminal cost
Phi_Org = 0.5*(   (y1-ref1)*pf(1,1)*(y1-ref1) + (y2-ref2)*pf(2,2)*(y2-ref2)  + (y3-ref3)*pf(3,3)*(y3-ref3)  );
Phi = vpa(Phi_Org,3);
...
L_Org = 0.5*(   (y1-ref1)*q(1,1)*(y1-ref1) + (y2-ref2)*q(2,2)*(y2-ref2) + (y3-ref3)*q(3,3)*(y3-ref3)    );
L = vpa(L_Org,3);
...
%      -u4-0;
%      (u1-0.1)*(u4-0.1);
%      py_data.model.Ts*Dx_lowDim_Sim(1)-1;
%      py_data.model.Ts*Dx_lowDim_Sim(2)-1;
%      py_data.model.Ts*Dx_lowDim_Sim(3)-1;
%      -py_data.model.Ts*Dx_lowDim_Sim(1)-0.1;
%      -py_data.model.Ts*Dx_lowDim_Sim(2)-0.1;
%      -py_data.model.Ts*Dx_lowDim_Sim(3)-0.1];
% C =  [u1-10;
%       u2-10;
%       u3-10;
%       u4-10;
...
if isempty(Phi)
    Phi = zeros(0,1);
    matlabFunction(Phi, 'file', 'valPhi','vars',[x, u, ref]);
else
    matlabFunction(Phi, 'file', 'valPhi','vars',{x, u, ref}); 
end
...
if isempty(L)
    L = zeros(0,1);
    matlabFunction(L, 'file', 'valL','vars',[x, u, ref]); 
else
    matlabFunction(L, 'file', 'valL','vars',{x, u, ref}); 
end
...
if isempty(Phi)
    ...
    matlabFunction(Phix, 'file', 'DPhiDx','vars',{x, u, ref});   
    ...
    matlabFunction(Phixx, 'file', 'D2PhiDxx','vars',{x, u, ref}); 
end
...
if isempty(L)
    ...
    matlabFunction(Lx, 'file', 'DLDx','vars',{x, u, ref});   

    for j=1:size_u
        Lu(1,j) = diff(L,(sprintf('u%1d',j)));
    end
    matlabFunction(Lu, 'file', 'DLDu','vars',{x, u, ref});   
    ...
    matlabFunction(Lxx, 'file', 'D2LDxx','vars',{x, u, ref}); 
    ...
    matlabFunction(Lxu, 'file', 'D2LDxu','vars',{x, u, ref}); 
    ...
    matlabFunction(Luu, 'file', 'D2LDuu','vars',{x, u, ref}); 
