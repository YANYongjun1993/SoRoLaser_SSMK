...
[ datafile_name , datafile_path ] = uigetfile( 'E:\Research\SRIL\14th\TroRevision\HardwareExperiemnt\SSMKwoDelay\SSMKLearning\systems\fromData\*.mat' , 'Choose data file for sysid...' );
data4sysid = load( [datafile_path , datafile_name] );
% load parameterized SSM model SSM_model.m
load E:\Research\SRIL\14th\TroRevision\HardwareExperiemnt\SSMKwoDelay\SSMKLearning\SSM_model.mat

f = generate_functions(data4sysid.sysid_class, ssm_model);
...
%  zeta  : The appended states.
%  p     : The original observables.

syms x u y z p zeta real
...
for i = 1:1:size(ssm_model.IMInfoCtrl.parametrization.tangentSpaceAtOrigin,1)
...
% chart map
chart = ssm_model.IMInfoCtrl.chart.map(z'); % Project down
% parameterization map
param = ssm_model.IMInfoCtrl.parametrization.map(y');
