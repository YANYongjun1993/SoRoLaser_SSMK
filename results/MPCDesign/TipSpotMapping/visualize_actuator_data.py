...
def save_data_to_mat(actual_coords, timestamps, pixel_coords=None, filename=None):
    """
    Save coordinate data and timestamps to .mat file
    
    Args:
        actual_coords: Array of actual coordinates (N, 2)
        timestamps: Array of timestamps (N,)
        pixel_coords: Array of pixel coordinates (N, 2), optional
        filename: Output filename, optional
    """
    if filename is None:
        current_date = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"actuator_motion_data_{current_date}.mat"
    ...
    mat_data = {
        ...
        'description': 'Actuator motion data extracted from MCAP/JSON file',
        'creation_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'data_points': len(actual_coords)
    }
    
    # 如果有像素坐标，也保存
    if pixel_coords is not None:
        mat_data['pixel_coords'] = pixel_coords
    
    # 添加一些有用的统计信息
    if len(actual_coords) > 0:
        mat_data['x_range'] = [np.min(actual_coords[:, 0]), np.max(actual_coords[:, 0])]
        mat_data['y_range'] = [np.min(actual_coords[:, 1]), np.max(actual_coords[:, 1])]
        mat_data['center_x'] = np.mean(actual_coords[:, 0])
        mat_data['center_y'] = np.mean(actual_coords[:, 1])
    
    if len(timestamps) > 0:
        # 转换时间戳为相对时间（秒）
        start_time = timestamps[0]
        relative_times = (timestamps - start_time) / 1e9
        mat_data['relative_times'] = relative_times
        mat_data['start_timestamp'] = start_time
        mat_data['duration_seconds'] = relative_times[-1] if len(relative_times) > 0 else 0
    
    try:
        # 保存为.mat文件
        savemat(filename, mat_data)
        print(f"数据已成功保存为MAT文件: {filename}")
        print(f"包含的数据:")
        print(f"  - actual_coords: {actual_coords.shape} (实际坐标)")
        print(f"  - timestamps: {timestamps.shape} (原始时间戳)")
        print(f"  - relative_times: {len(relative_times)} (相对时间，秒)")
        if pixel_coords is not None:
            print(f"  - pixel_coords: {pixel_coords.shape} (像素坐标)")
        print(f"  - 轨迹范围: X=[{mat_data['x_range'][0]:.2f}, {mat_data['x_range'][1]:.2f}], Y=[{mat_data['y_range'][0]:.2f}, {mat_data['y_range'][1]:.2f}]")
        print(f"  - 持续时间: {mat_data['duration_seconds']:.2f} 秒")
...
def main():
    ...
    支持从MCAP文件或JSON文件读取数据，并保存为MAT文件
    """
    # 配置文件路径
    ...
    if timestamps is not None and len(pixel_coords) > 0:
        ...
        # 保存数据为MAT文件
        print("\n保存数据为MAT文件...")
        save_data_to_mat(actual_coords, timestamps, pixel_coords)
