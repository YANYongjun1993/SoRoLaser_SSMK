classdef ksysid
    ...
    properties
        ...
        obs_type;   % cell array of the types of observables to include in model:poly, fourier, fourier-sparser, gaussian, hermite
...
%             obj.valdata = data4val;
        ...
        function [ data_scaled , obj ] = get_scale( obj , data )
            ...
            y_max = max( data.y );
            u_max = max( data.u );
            u_amp = max(u_max,-u_min); % amplitude of input signal
            u_max = u_amp; % set max to amplitude, so that it is symmetric around 0
            u_min = -u_amp; % set min to negative amplitude
...
%             [ ysim , tsim , zsim ] = lsim(model.sys, ureal , treal , z0); % Don't use lsim. Might be doing weird stuff
        ...
        function [ yDataPredHoriz ] = valOverHorizion_lowDim( obj , ObsEmdbCtrlVal)
            % if no model id is provided, use first one in the model cell array
            if nargin < 3 && ~iscell(obj.candidates)
                mod = obj.candidates;
            elseif nargin < 3
                mod = obj.candidates{1};
            else
            ...
            Nhoriz = 11;
            nEval = length(obj.valdata{1,1}.t)-Nhoriz;
            yDataPredHoriz = cell(1,5);
            iRow = 1;
            for iTime = 1:nEval
                uHoriz = u(:,iTime:iTime+Nhoriz);
                tHoriz = t(iTime:iTime+Nhoriz);
                yHoriz = y(:,iTime:iTime+Nhoriz);
                xHoriz = x(:,iTime:iTime+Nhoriz);
                valSection = struct('u',uHoriz','t',tHoriz','y',yHoriz','x',xHoriz);
                results = obj.val_model( mod , valSection );
                % uFun = @(t) transpose(interp1(tHoriz, transpose(uHoriz), t));
                % [iTimeRec, ietaRec] = ode15s(@(t,x) R(t,x,uFun(t)),tHoriz,xi(:,iTime));
                % Lift
                % iyRec = liftTrajectories(ssm_model.IMInfoCtrl, {iTimeRec, transpose(ietaRec)});
                
                yDataPredHoriz{iRow,1} = obj.scaleup.y(results.sim.y);
                yDataPredHoriz{iRow,2} = results.sim.y;
                yDataPredHoriz{iRow,3} = results.sim.t;
                yDataPredHoriz{iRow,4} = xHoriz';
                yDataPredHoriz{iRow,5} = results;
                iRow = iRow + 1;
            end

            obj.valOverHorizion_plot(yDataPredHoriz,obj.valdata{1,1});

        end

        % Validate the derived model over short predictive horizion
        function [ yDataPredHoriz ] = valOverHorizion_plot( obj , yDataPredHoriz, validateData)
             % validation at the reduced coordinates
            idxPlotCL = 50;
            labelsRD = ["$\alpha_1$", "$\alpha_2$"];
            % Plot time series
            ...
            for iObs = 1:2
                ...
                for iTraj=1:idxPlotCL:size(yDataPredHoriz,1)
                    plot(yDataPredHoriz{iTraj,3}, yDataPredHoriz{iTraj,2}(:,iObs),'Linewidth',2);
                end
            end
        end

        % Validate the derived model over short predictive horizion
        function [ yDataPredHoriz ] = valOverHorizion_obs( obj , model_id)
            % if no model id is provided, use first one in the model cell array
            if nargin < 2 && ~iscell(obj.candidates)
                mod = obj.candidates;
            elseif nargin < 2
            ...
            Nhoriz = 11;
            nEval = length(obj.valdata{1,3}.t)-Nhoriz;
            yDataPredHoriz = cell(1,5);
            iRow = 1;
            for iTime = 1:nEval
                uHoriz = u(:,iTime:iTime+Nhoriz);
                ...
                yDataPredHoriz{iRow,2} = results.sim.y;
                yDataPredHoriz{iRow,3} = results.sim.t;
                yDataPredHoriz{iRow,4} = obj.scaleup.y(yHoriz');
                yDataPredHoriz{iRow,5} = results;
                iRow = iRow + 1;
            end

            obj.valOverHorizion_plot(yDataPredHoriz,obj.valdata{1,3});
