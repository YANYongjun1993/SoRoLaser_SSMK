...
refBtm = ref(:,end-33:end-1);
refExt{1,2} = [refBtm,ref];
refExt{1,1} = 0:0.01:0.01*(size(refExt{1,2},2) - 1);
% perform delay-embedding
SSMDim = 4;
overEmbed = 32;
[refDelayEmbed, opts_embd] = coordinatesEmbeddingControl(refExt, SSMDim, 'OverEmbedding', overEmbed);
embedDim = size(refDelayEmbed{1,2},1);
...
for i = 1:size(refDelayEmbed{1,2},2)
    lowDim(:,i) = chart(refDelayEmbed{1,2}(:,i)');
end
for i = 1:size(refDelayEmbed{1,2},2)
...
for i = 1:figRow
    subplot(figRow,1,i);
    plot(refDelayEmbed{1,1},refDelayEmbed{1,2}(outdofsDelay(i),:),'Linewidth',1)
    hold on
    plot(refDelayEmbed{1,1},refReconst(outdofsDelay(i),:),'k:','Linewidth',2)
end
% 保存最终的ref数据
