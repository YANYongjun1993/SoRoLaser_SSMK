...
def visualize_multiple_trajectories(results_dict):
    ...
    timestamps, actual_coords = slice_motion_data(timestamps, actual_coords, 153, 615)
    ...
    plot_filename = f"SSMP_actual_coordinate_trajectory_{current_date}.png"
...
def visualize_time_series(results_dict):
    ...
    # Save the plot
    current_date = datetime.now().strftime("%Y%m%d_%H%M%S")
    plot_filename = f"SSMP_time_series_coordinates_{current_date}.png"
    plt.savefig(plot_filename, dpi=600, bbox_inches='tight')
...
def main():
    ...
    file_configs = [
        {
            'mcap_file': "rosbag_20250816_173516_0.mcap",
            'mat_file': "SSMP.mat",  # Changed from json_file to mat_file
            'label': "SSMP",
            'color': plasma_color,
            'marker': 'o'
        }
